...
class ClipItStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        gpu_user_data.add_commands(
            ...
            "# Ensure ECS service is enabled and started (not just restarted)",
            "systemctl enable ecs",
            "systemctl start ecs",
            "# Wait for ECS agent to actually start (with timeout)",
            "for i in {1..30}; do",
            "  if systemctl is-active ecs >/dev/null 2>&1; then",
            "    echo 'ECS agent is running!'",
            "    break",
            "  fi",
            "  echo \"Waiting for ECS agent to start... ($i/30)\"",
            "  sleep 2",
            "done",
            "# Verify it started",
            "systemctl status ecs --no-pager || echo 'ECS agent failed to start'",
            "echo '=== Setup Complete ==='",
            "date"
        )
